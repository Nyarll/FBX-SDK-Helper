...
namespace FBX_LOADER
{
	...
	HRESULT CFBXLoader::LoadFBX(const char* file_name, const eAXIS_SYSTEM axis)
	{
		if (!file_name)
			return E_FAIL;
		...
		this->InitializeSdkObjects(mSdkManager, mScene);
		if (!mSdkManager)
			return E_FAIL;
		...
		if (!mSdkManager->GetIOPluginRegistry()->DetectReaderFileFormat(file_name, lFileFormat))
		{
			lFileFormat = mSdkManager->GetIOPluginRegistry()->FindReaderIDByDescription("FBX binary (*.fbx)");
		}
		if (!mImporter || mImporter->Initialize(file_name, lFileFormat) == false)
			return E_FAIL;
	...
	void CFBXLoader::SetFbxColor(FBX_MATERIAL_ELEMENT& destColor, const FbxDouble3 srcColor)
	{
		destColor.a = 1.0f;
	...
	void CFBXLoader::CopyMaterialData(FbxSurfaceMaterial* mat, FBX_MATERIAL_NODE* destMat)
	{
		if (!mat)
			return;
